...
@section('title', 'Edit Package - Marketing')
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control" id="title_ar" name="title_ar"
                                                   value="{{ old('title_ar', $package->title_ar) }}" required dir="rtl">
                                        </div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <textarea class="form-control" id="description_ar" name="description_ar"
                                                      rows="3" dir="rtl">{{ old('description_ar', $package->description_ar) }}</textarea>
                                        </div>
                                        ...
                                        <div class="mb-3">
                                            <label for="image" class="form-label">New Package Image</label>
                                            <input type="file" class="form-control" id="image" name="image"
                                                   accept="image/*">
                                            <div class="form-text">Leave empty to keep current image. Recommended size: 400x300px. Max size: 2MB</div>
                                        </div>
                                                        ...
                                                        <div class="ms-3">
                                                            <div class="input-group" style="width: 200px;">
                                                                <select class="form-select form-select-sm discount-type-select"
                                                                        ...
                                                                        {{ in_array($product->id, $selectedProducts) ? '' : 'disabled' }}>
                                                                    <option value="percentage" {{ (isset($reductions[$product->id]) && $reductions[$product->id]['type'] == 'percentage') ? 'selected' : (!isset($reductions[$product->id]) ? 'selected' : '') }}>%</option>
                                                                    <option value="fixed" {{ (isset($reductions[$product->id]) && $reductions[$product->id]['type'] == 'fixed') ? 'selected' : '' }}>$</option>
                                                                </select>
                                                                <input type="number" class="form-control form-control-sm reduction-input"
                                                                       name="reductions[]" min="0" step="0.1"
                                                                       placeholder="0"
                                                                       value="{{ isset($reductions[$product->id]) ? $reductions[$product->id]['value'] : '' }}"
                                                                       {{ in_array($product->id, $selectedProducts) ? '' : 'disabled' }}>
                                                            </div>
                                                        </div>
                                                    </div>
                            ...
                            <div class="col-12">
                                <div class="d-flex justify-content-end gap-2">
                                    ...
                                    <button type="submit" class="btn btn-primary">
                                        <i class="fa fa-save me-1"></i>Update Package
                                    </button>
                                </div>
                            </div>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    checkboxes.forEach((checkbox, index) => {
        ...
        // Initialize disabled state for unchecked boxes
        if (!checkbox.checked) {
