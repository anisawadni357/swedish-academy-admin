...
class UpdateAffiliatePartnerRequest extends FormRequest
{
    ...
    {
        $partnerId = $this->route('affiliatePartner')->id ?? $this->route('affiliate_partner')->id ?? null;

        return [
            'name' => 'required|string|max:255',
            'email' => 'required|email|unique:affiliate_partners,email,' . $partnerId,
    ...
    {
        Log::error('Affiliate partner update validation failed', [
