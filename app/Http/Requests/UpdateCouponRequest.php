...
class UpdateCouponRequest extends FormRequest
{
    ...
    {
        // Convert checkbox "1" or "on" values to boolean, ensure unchecked = false
        $booleanFields = [
            'is_active',
            'is_stackable',
            'auto_apply',
        ...
        foreach ($booleanFields as $field) {
            ...
            $merge[$field] = $this->has($field) && ($value === '1' || $value === 'on') ? true : false;
        }
        ...
        if ($this->has('custom_message') && !empty($this->input('custom_message'))) {
            $merge['description'] = $this->input('custom_message');
        }
    ...
    public function rules(): array
    {
        ...
        $rules = [
            'nom' => 'required|string|max:255|unique:coupons,nom,' . $couponId,
            'code' => 'required|string|max:50|unique:coupons,code,' . $couponId . '|regex:/^[A-Z0-9]+$/',
    ...
    public function messages(): array
    {
        return [
            ...
            'code.unique' => 'This coupon code is already taken by another coupon.',
    ...
    {
        Log::error('Coupon update validation failed', [
